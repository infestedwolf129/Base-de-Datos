...
if ($new_cant > $cant){
    $conn -> query("UPDATE hoteles SET habitaciones_d = habitaciones_d - '$new_cant - $cant' WHERE nombre_h = '$nom_compra'");
}else if ($new_cant < $cant){
    $conn -> query("UPDATE hoteles SET habitaciones_d = habitaciones_d + '$cant - $new_cant' WHERE nombre_h = '$nom_compra'");
}
?>
